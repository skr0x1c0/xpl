...
xpl_allocator_prpw_t xpl_allocator_prpw_create(const struct sockaddr_in* addr, size_t num_allocs) {
    ...
    xpl_allocator_prpw_t allocator = (xpl_allocator_prpw_t)malloc(sizeof(struct xpl_allocator_prpw));
...
int xpl_allocator_prpw_filter(xpl_allocator_prpw_t allocator, size_t offset, size_t count, xpl_allocator_prpw_data_filter filter, void* filter_ctx, int64_t* found_idx_out) {
    ...
    int error = xpl_dispatch_apply(&allocator->backends[backend_start_idx], sizeof(smb_nic_allocator), backend_count, &found_idx, ^(void* ctx, void* data, size_t index) {
        ...
        xpl_assert(infos.num_of_nics == MAX_NICS_PER_BACKEND);
        for (int i=MAX_NICS_PER_BACKEND - 1; i>=(int)(MAX_NICS_PER_BACKEND - backend_num_nics); i--) {
            ...
            xpl_assert(nic_idx == MAX_NICS_PER_BACKEND - i - 1);
            size_t nic_alloc_start_idx = backend_alloc_start_idx +  (nic_idx * MAX_IPS_PER_NIC);
            size_t nic_alloc_end_idx = xpl_min(nic_alloc_start_idx + MAX_IPS_PER_NIC, alloc_cursor) - 1;
...
int xpl_allocator_prpw_read(xpl_allocator_prpw_t allocator, size_t alloc_index, sa_family_t* family) {
    size_t alloc_cursor = allocator->alloc_cursor;
    ...
    size_t nic_alloc_count = xpl_min(MAX_IPS_PER_NIC, alloc_cursor - ((backend_idx * MAX_ALLOCS_PER_BACKEND) + (nic_idx * MAX_IPS_PER_NIC)));
    ...
    xpl_assert(info.num_of_nics == MAX_NICS_PER_BACKEND);
    xpl_assert(nic_idx < info.num_of_nics);

    struct nic_properties props = info.nic_props[MAX_NICS_PER_BACKEND - nic_idx - 1];
    xpl_assert(props.if_index == nic_idx);
    xpl_assert(props.num_of_addrs >= nic_alloc_count);

    *family = props.addr_list[local_idx].addr_family;
    return 0;
}

int xpl_allocator_prpw_trim_backend_count(xpl_allocator_prpw_t allocator, size_t offset, size_t count) {
...
int xpl_allocator_prpw_release_containing_backend(xpl_allocator_prpw_t allocator, int64_t alloc_index) {
    ...
    return xpl_allocator_prpw_trim_backend_count(allocator, alloc_index / MAX_ALLOCS_PER_BACKEND, 1);
}
