...
int xpl_util_tcc_remove_entry(sqlite3* db, const char* client, const char* service) {
...
int xpl_util_tcc_add_entry(sqlite3* db, const char* client, const char* service, int client_type, int auth_value, int auth_reason) {
    sqlite3_stmt* stmt;
...
int xpl_util_tcc_modify_db(const char* db_path, const char* client, const char* service, int client_type, int auth_value, int auth_reason) {
    ...
    error = xpl_util_tcc_remove_entry(db, client, service);
    if (error) {
        xpl_log_error("removing service: %s, client: %s from db: %s failed, err: %d", service, client, db_path, error);
        goto exit;
    }
    
    error = xpl_util_tcc_add_entry(db, client, service, client_type, auth_value, auth_reason);
    if (error) {
...
int xpl_util_tcc_authorize(const char* db, const char* client, const char* service) {
    return xpl_util_tcc_modify_db(db, client, service, 0, 2, 4);
}
