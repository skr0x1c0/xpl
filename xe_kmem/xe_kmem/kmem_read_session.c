...
void kmem_read_session_fragment_kext_32(const struct sockaddr_in* smb_addr, xe_allocator_nrnw_t nrnw_allocator) {
...
void kmem_read_session_build_sock_addr(kmem_read_session_t session, const struct sockaddr_in* smb_addr) {
    ...
    /// `xe_kheap_alloc` reads the address of allocated memory by reading the field `addr` of
...
/// the `struct smbiod` has size 704 but we can only allocate upto 256 bytes using `xe_kheap_alloc`.
...
kmem_read_session_t kmem_read_session_create(const struct sockaddr_in* smb_addr) {
    ...
    xe_allocator_nrnw_allocate(nrnw_allocator, 32, XE_PAGE_SIZE / 32 * 16);
...
void kmem_read_session_destroy(kmem_read_session_t* session_p) {
    ...
    xe_allocator_prpw_destroy(&session->sock_addr_alloc.element_allocator);
    xe_allocator_prpw_destroy(&session->sock_addr_entry_alloc.element_allocator);
    xe_allocator_prpw_destroy(&session->iod_alloc.element_allocator);
    xe_allocator_prpw_destroy(&session->session_alloc.element_allocator);
    free(session);
    *session_p = NULL;
}
