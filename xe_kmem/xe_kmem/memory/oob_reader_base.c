...
void oob_reader_base_read(const struct oob_reader_base_args* args, char* server_nb_name, uint32_t* server_nb_name_size, char* local_nb_name, uint32_t* local_nb_name_size) {
    ...
    /// Embed custom instructions for xe_smbx server. See
    /// `RequestHandler::handleNetbiosSsnRequest` method in xe_smbx/main.swift
    ...
    /// Tell xe_smbx server to respond with `NB_SSN_NEGRESP` response code whiich will
    ...
    /// Execute the `SMBIOC_NEGOTIATE` leading to OOB read data being sent to xe_smbx
    /// server
    ...
    /// Read the local and server netbios names sent to xe_smbx server which will be containing
    /// the OOB read data
