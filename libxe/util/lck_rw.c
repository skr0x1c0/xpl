...
xe_util_lck_rw_t xe_util_lck_rw_lock_exclusive(uintptr_t lock) {
    ...
    dispatch_async(xe_dispatch_queue(), ^() {
        ...
        /// the method `xe_util_lck_create_nstat_controls_cycle` has created a cycle in
...
int xe_util_lck_rw_wait_for_contention(xe_util_lck_rw_t util, uintptr_t thread, uintptr_t* lr_stack_ptr) {
    /// ~  try for 10 seconds
    int max_tries = 1000;
